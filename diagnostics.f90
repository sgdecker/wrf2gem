...
!orig   z(k) = lin_int(-log(pw(k)), -log(pw(k+1)), zs(k), zs(k+1), -log(pu(k)))
  ...
  function storm_rel_hel(u, v, z, u10m, v10m, ter, heights) result (srh)
    ...
    srhs = (uInt - spread(uStorm, dim=3, ncopies=numLevs)) * xVort +  &
         (vInt - spread(vStorm, dim=3, ncopies=numLevs)) * yVort
  ...
  ! calcgust is adapted from an algorithm used by wrfpost.  Given as input the
  ! u and v components of the wind (m/s), the height above ground at each
  ! gridpoint (hagl; m), the u and v components of the wind at 10m (u10m,
  ! v10m; m/s), and the height of the boundary layer (zpbl; m), the wind gust
  ! (m/s) is calculated.
  function calcgust(u, v, hagl, u10m, v10m, zpbl) result(gust)
    real, dimension(:,:,:,:), intent(in) :: u, v, hagl
    real, dimension(:,:,:),   intent(in) :: u10m, v10m, zpbl
    real, dimension(size(u,1),size(u,2),size(u,4)) :: gust

    real, parameter :: r2000 = 1./2000.

    real, dimension(size(u,1),size(u,2),size(u,4)) :: sfcwind, pblwind
    real, dimension(2) :: haglBounds, windBounds
    real :: uInt, vInt
    integer :: nx, ny, nt, i, j, t

    nx = size(u,1) ; ny = size(u,2) ; nt = size(u,4)

    sfcwind = sqrt(u10m**2 + v10m**2)

    do t = 1, nt
       do j = 1, ny
          do i = 1, nx
             if (hagl(i,j,1,t) > zpbl(i,j,t)) then
                uInt = u(i,j,1,t)
                vInt = v(i,j,1,t)
             else
                call find_bounds(hagl(i,j,:,t), u(i,j,:,t), zpbl(i,j,t),  &
                     haglBounds, windBounds)
                uInt = lin_int(haglBounds(1), haglBounds(2), windBounds(1),  &
                     windBounds(2), zpbl(i,j,t))
                call find_bounds(hagl(i,j,:,t), v(i,j,:,t), zpbl(i,j,t),  &
                     haglBounds, windBounds)
                vInt = lin_int(haglBounds(1), haglBounds(2), windBounds(1),  &
                     windBounds(2), zpbl(i,j,t))
             end if
             pblwind(i,j,t) = sqrt(uInt**2 + vInt**2)
          end do
       end do
    end do

    gust = sfcwind + (pblwind - sfcwind) * (1 - min(.5,r2000*zpbl))
  end function calcgust
